# -*- coding: utf-8 -*-
"""icmp_flood_final.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1dIbUm81Jln0m381UdeaAFJoWVgTO-vuD
"""
...
# Đọc dữ liệu từ file PCAP ddos.pcap < CIC IoT DATASET2023 >
pcap_file_path = '/content/drive/My Drive/ddos.pcap'

# dọc file pcap ddos.pcap
packets = rdpcap(pcap_file_path)

# Khởi tạo danh sách để lưu các đặc trưng của gói tin ICMP flood
packet_data = []
...
for packet in packets:
    if IP in packet and ICMP in packet:
        ...
        # Kiểm tra và xử lý None
        icmp_id = packet[ICMP].id if packet[ICMP].id is not None else 0
        icmp_seq1 = packet[ICMP].seq if packet[ICMP].seq is not None else 0
        if packet.haslayer(ICMP) and hasattr(packet[ICMP], 'seq'):
            raw_data = bytes(packet[ICMP])
            if len(raw_data) >= 2:
                icmp_seq2 = struct.unpack('!H', raw_data[2:4])[0]  # Giả sử seq2 nằm ở byte thứ 6-7
            else:
                icmp_seq2 = 0
        else:
            icmp_seq2 = 0
        ...
        icmp_info = f"Echo (ping) request  id=0x{icmp_id:x}, seq={icmp_seq1}/{icmp_seq2}, ttl={ttl} (no response found!)"
        packet_data.append([time, src_ip, dst_ip, protocol, size, icmp_info])

# Tạo DataFrame từ danh sách các đặc trưng
df = pd.DataFrame(packet_data, columns=['time', 'src_ip', 'dst_ip', 'protocol', 'size', 'icmp_info'])
# Lưu DataFrame vào file CSV
file_path = '/content/drive/My Drive/icmp_flood.csv'
df.to_csv(file_path, index=False)
print("Đã lưu các đặc trưng của các gói tin ICMP flood vào file CSV.")
...
print(f"Tổng số gói tin icmp_flood trong tệp dataset là: {total_rows}")
...
# Tạo gói tin mới từ dữ liệu giả và lưu vào tệp PCAP
packets = []
for index, row in combined_df.iterrows():
    packet = Ether()/IP(src=row['src_ip'], dst=row['dst_ip'], ttl=int(row['icmp_info'].split('ttl=')[1].split(' ')[0]))/ICMP(type=8, id=random.randint(0, 65535), seq=random.randint(0, 65535))
    packets.append(packet)
wrpcap('/content/drive/My Drive/icmp_flood_attrack.pcap', packets)